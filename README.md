...
**Bots:** same physics as you — no rubber-band, no extra grip. Cheap look-ahead: they wind the longs, lift for tight radii (180 / chicane / sweeper), and peel LEFT for one box on lap 3–4. **BowieKnife99** is the aggressive rival (late-ish brakes, occasional 180 overshoot). **Hall Monitor** is tidy and boxes on time. The rest are a bit messy. Locked to the grid until GO; getaways can be sloppy. A clean player can win.
...
`index.html` is marked `no-store`. JS/CSS are versioned (`?v=mp26`) so a **normal reload** picks up the lobby.
