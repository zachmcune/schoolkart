...
**Bots:** same physics as you — no extra grip. Cheap look-ahead: they wind the longs, lift for tight radii (180 / chicane / sweeper), and peel LEFT for one box on lap 3–4. **BowieKnife99** is the hunter — he winds at the cap, late-brakes, and if you pull a gap he stays on the ribbon and reels you in (no teleport, no extra grip). Close up he divebombs, shoves, and sideswipes (you first). If he’s ahead he **blocks the pass** — covers your lane and keeps racing, never a U-turn ram. Same bash physics. **Hall Monitor** is tidy and boxes on time. The rest are a bit messy. Only Bowie hunts. Locked to the grid until GO; getaways can still fluff. A clean player can win.
...
`index.html` is marked `no-store`. JS/CSS are versioned (`?v=mp86`) so a **normal reload** picks up the lobby.
