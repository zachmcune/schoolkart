...
**Phones (landscape):** right half = gas · left half = brake · tilt to steer (held-landscape roll, small deadzone, smoothed — a modest lean is full lock). Tiny **R** is reverse for a pit overshoot. iPhone needs a **Tap for tilt steer** the first time. First phone load shows: `Right gas · Left brake · Tilt to steer`.
...
`index.html` is marked `no-store`. JS/CSS are versioned (`?v=mp93`) so a **normal reload** picks up the lobby.
