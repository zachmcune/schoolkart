...
var code = [
  ...
  sliceFn("pickPrey"),
  sliceFn("followLimit"),
  src.match(/var SIDE_ROOM = [0-9.]+;/)[0],
  src.match(/var SIDE_EASE = [0-9.]+;/)[0],
  sliceFn("sideNudge"),
  sliceFn("laneClear"),
  sliceFn("ribbonOff"),
  ...
  "    stat.push({ name: names[i], grass: 0, offTrack: 0, reverseT: 0, maxSpeed: 0, maxOff: 0, pitStops: 0, wasBox: false, emptyT: 0, still: 0, contacts: 0, contactT: 0, peelT: 0, clearT: 99 });",
  "  }",
  "  raceTime = 0;",
  "  var resetLog = [];",
  "  var carLog = [];",
  ...
  // A shunt sets aiHitT fresh on every frame the bodywork is touching,
  // so counting those frames measures how long cars leaned on each other
  // rather than how often they hit — and a grind that skips a frame here
  // and there counts double. One incident is one burst with clean air
  // either side of it.
  "      if (r.aiHitT > 0) {",
  "        if (st.clearT > 0.6) st.contacts += 1;",
  "        st.clearT = 0;",
  "        st.contactT += dt;",
  "      } else {",
  "        st.clearT += dt;",
